...
function renderPolybiusDraft(container, ciphertext, state) {
  ...
  // Информация о заменах (для 6×6 все буквы уникальны, кроме пробела)
  const extraInfo = {
    ' ': '␣ пробел'
  };
  ...
  function findInGrid(coords) {
    ...
    if (row >= 0 && row < 6 && col >= 0 && col < 6 && gridData[row][col]) {
  ...
  container.innerHTML = `
    <div class="label">// SCRATCHPAD: Квадрат Полибия (6×6)</div>
    ...
    <div style="margin-bottom: 1.5rem; overflow-x: auto;">
      ...
      <div style="display: inline-block; border: 1px solid #333; background: rgba(0,0,0,0.3);">
        <!-- Заголовок колонок 1-6 -->
        <div style="display: grid; grid-template-columns: 35px repeat(6, 50px); gap: 0; text-align: center; font-size: 0.7rem; color: #666; border-bottom: 1px solid #333;">
          <div style="padding: 6px;"></div>
          <div style="padding: 6px;">1</div><div style="padding: 6px;">2</div><div style="padding: 6px;">3</div>
          <div style="padding: 6px;">4</div><div style="padding: 6px;">5</div><div style="padding: 6px;">6</div>
        </div>
        ${gridData.map((row, rowIdx) => `
          <div style="display: grid; grid-template-columns: 35px repeat(6, 50px); gap: 0; text-align: center; ${rowIdx < 5 ? 'border-bottom: 1px solid #222;' : ''}">
            <div style="padding: 10px 4px; color: #666; font-size: 0.7rem;">${rowIdx + 1}</div>
            ${row.map((cell, colIdx) => `
              <div style="padding: 10px 4px; color: var(--text-primary); font-size: 1rem; ${colIdx < 5 ? 'border-right: 1px solid #222;' : ''}">
                ${cell === ' ' ? '␣' : cell}
              </div>
            `).join('')}
          </div>
      ...
      <div style="margin-top: 0.5rem; font-size: 0.7rem; color: #555;">
        ✦ Координаты: строка (1-6) и столбец (1-6)<br>
        ✦ Пробел обозначен как ␣ и имеет координаты 66
      </div>
    </div>
    ...
    <div style="margin-bottom: 1rem;">
      ...
      <div id="polybiusInputRow" style="display: flex; gap: 8px; flex-wrap: wrap; align-items: flex-end;">
        ${pairs.map((pair, idx) => {
          const expectedChar = plaintext ? (plaintext[idx] || '') : '';
          ...
          return `<div style="display: flex; flex-direction: column; align-items: center; gap: 3px;" title="Координаты ${pair} → ${gridLetter}">
            ...
            <input type="text" 
                   ...
                   style="width: 44px; height: 44px; text-align: center; font-size: 1.3rem; border: 1px solid #333; background: transparent; color: var(--text-primary);" />
          </div>`;
        }).join('')}
      </div>
    </div>
...
function renderPlayfairDraft(container, ciphertext, state) {
  ...
  console.log('=== ПЛЕЙФЕР ===');
  console.log('Ключ:', taskKey);
  console.log('Уровень:', state?.task?.level);
  ...
  function buildMatrix6x6(key) {
    ...
    // Собираем: сначала ключ, потом алфавит
    const result = [...clean];
